...
import { useDashConect } from '../conect/dashconect'; // 🔗 Conexão com Dashboard
...
const validateAndFormatDate = (input: string): string => {
  ...
  let formattedDate = '';
  if (cleanInput.length > 0) formattedDate += cleanInput.substring(0, 2);
  if (cleanInput.length > 2) formattedDate += '/' + cleanInput.substring(2, 4);
  if (cleanInput.length > 4) formattedDate += '/' + cleanInput.substring(4, 8);
  ...
  return (
    <div className="bg-white p-3 shadow-sm rounded-lg mb-3 border border-gray-200">
      ...
      <div className="flex justify-between items-start mb-2">
        <h4 className="text-sm font-semibold truncate text-gray-800">{tarefa.titulo}</h4>
      ...
      {tarefa.descricao && <p className="text-xs text-gray-600 mb-3">{tarefa.descricao}</p>}
      ...
      <div className="flex justify-between items-center text-xs mt-3">
        ...
        <div className="flex items-center space-x-1">
          <Pencil
            size={14}
            className="text-gray-400 cursor-pointer hover:text-blue-500 mr-2"
...
const TaskModal: React.FC<ModalProps> = ({ isOpen, onClose, onSubmit, initialTask }) => {
  ...
  return (
    <div className="fixed inset-0 bg-gray-600 bg-opacity-75 z-50 transition-opacity duration-300">
      <div
        className={`fixed top-0 right-0 h-full w-full max-w-md bg-white shadow-2xl transition-transform duration-300 transform ${
        ...
        <div className="p-6 h-full flex flex-col">
          ...
          <div className="flex justify-end space-x-3 pt-4 border-t mt-auto">
            <button
              type="button"
              onClick={onClose}
              className="px-5 py-2 text-gray-700 rounded-md hover:bg-gray-100"
            >
              Cancelar
            </button>
            <button
              ...
              className="px-5 py-2 bg-blue-600 text-white font-semibold rounded-md hover:bg-blue-700 transition duration-150"
            >
...
export default function Tarefas() {
  const { data, adicionarTarefa, atualizarTarefa, excluirTarefa } = useDashConect();
  ...
  const handleSaveTask = (newTask: Tarefa) => {
    if (editingTask) {
      atualizarTarefa(editingTask.id, newTask);
    } else {
      adicionarTarefa({
        titulo: newTask.titulo,
        responsavel: newTask.responsavel,
        prazo: new Date(),
        progresso: newTask.status === 'Concluído' ? 100 : 0,
        status:
      ...
      <div className="flex space-x-6 overflow-x-auto pb-4">
        {STATUS_COLUMNS.map(column => (
          <div key={column.name} className="flex-shrink-0 w-80">
            <div
              className={`flex items-center justify-between p-3 rounded-t-lg text-white ${column.color}`}
            >
              ...
              <span className="bg-white text-gray-800 rounded-full h-6 w-6 flex items-center justify-center text-xs font-bold">
                {
                  data.tarefas.filter(
                    t =>
                      (column.name === 'A Fazer' && t.status === 'pendente') ||
                      (column.name === 'Em andamento' && t.status === 'andamento') ||
                      (column.name === 'Concluído' && t.status === 'concluido')
                  ).length
                }
              </span>
            </div>
                ...
                .map(tarefa => (
                  <TaskCard
                    ...
                    tarefa={{
                      ...
                      descricao: '',
                      data: tarefa.prazo.toLocaleDateString(),
